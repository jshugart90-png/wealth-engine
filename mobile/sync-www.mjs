...
import {
  ...
  wealthHubPushInlineScript,
  wealthHubPortfolioCsvExportScript,
} from "./shared/push.mjs";
import { iapClientScript, iapBarHtml } from "./shared/iap.mjs";
import { generateWealthHubIndexHtml } from "./shared/portfolio-catalog.mjs";
...
function syncWealthHub() {
  const www = join(mobileRoot, "wealth-hub", "www");
  mkdirSync(www, { recursive: true });
...
.iap button{background:#6366f1;color:#fff;border:none;border-radius:8px;padding:10px 16px;font-weight:700;cursor:pointer;font-size:14px;margin:4px}
.iap #we-iap-premium-badge{display:none;background:#22c55e;color:#052e16;padding:4px 10px;border-radius:999px;font-size:12px;font-weight:700;margin-left:8px}
...
<p class="footer"><a href="privacy.html">Privacy</a> · <a href="https://wealth-engine-0qlj.onrender.com/">View portfolio online</a></p>
...
<script>${iapScript}</script>
  ...
  const hubDist = join(dist, "wealth-hub");
  mkdirSync(hubDist, { recursive: true });
  writeFileSync(join(hubDist, "index.html"), indexHtml);

  const copyTargets = [
    { src: join(dist, "games"), dest: join(www, "games"), recursive: true },
    { src: join(dist, "comparestack"), dest: join(www, "comparestack"), recursive: true },
    { src: join(dist, "billsnap"), dest: join(www, "billsnap"), recursive: true },
    { src: join(dist, "bundles"), dest: join(www, "bundles"), recursive: true },
  ];

  for (const { src, dest, recursive } of copyTargets) {
    if (!existsSync(src)) {
      console.error(`Missing ${src} — run npm run build first`);
      process.exit(1);
    }
    cpSync(src, dest, { recursive: !!recursive });
  }
  ...
  console.log(`Synced wealth-hub → mobile/wealth-hub/www (${apps.length} apps)`);
}
...
if (target === "wealth-hub" || target === "all") syncWealthHub();
